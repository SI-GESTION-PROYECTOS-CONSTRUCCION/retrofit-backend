...
public interface ProgressReportResourceRepository extends JpaRepository<ProgressReportResource, Long> {
    ...
    Double calculateActualCostByProjectId(@Param("projectId") Long projectId);
    ...
    @Query("SELECT COALESCE(SUM(prr.realQuantity * r.basePrice), 0.0) " +
            ...
            "JOIN prr.progressReport pr " +
            "JOIN prr.resource r " +
            "WHERE pr.projectItem.id = :projectItemId")
    Double calculateActualCostByProjectItemId(@Param("projectItemId") Long projectItemId);
    ...
    @Query("SELECT TYPE(r), COALESCE(SUM(prr.realQuantity * r.basePrice), 0.0) " +
            ...
            "WHERE pi.project.id = :projectId " +
            "GROUP BY TYPE(r)")
    List<Object[]> calculateActualCostByResourceType(@Param("projectId") Long projectId);
    ...
    List<Object[]> getActualCostByDate(@Param("projectId") Long projectId);
}